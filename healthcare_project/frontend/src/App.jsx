...
function RiskBadge({ value, fieldKey }) {
  ...
  if (!t) return null
  const isHigh = value >= t.danger
  const isWarn = value >= t.warn && value < t.danger
...
// ── Confetti burst on low-risk result
function useConfetti(trigger) {
  useEffect(() => {
    ...
    const pieces = Array.from({ length: 80 }, () => ({
      ...
      r: Math.random() * 7 + 3,
      d: Math.random() * 80 + 10,
      color: ['#10b981', '#2563eb', '#0f9f8f', '#38bdf8', '#a7f3d0'][Math.floor(Math.random() * 5)],
    ...
    const draw = () => {
      ...
      if (count < 180) requestAnimationFrame(draw)
      else canvas.remove()
    }
    draw()
...
function App() {
  ...
  async function handleRunPrediction() {
    ...
    try {
      ...
      setPredStep(2)
      setApiStatus('online')
      if (data.best_model.prediction === 0) setConfettiKey(Date.now())
      const entry = {
        id: Date.now(),
  ...
  if (booting) return (
    <main className="welcome-screen">
      <section className="welcome-card">
        ...
        <h1>Healthcare Prediction System</h1>
        <div className="boot-steps">
          {['Initializing models...', 'Loading patient data...', 'Preparing dashboard...', 'Ready!'].map((s, i) => (
            <div key={s} className={`boot-step ${bootStep > i ? 'done' : bootStep === i ? 'active' : ''}`}>
              <span className="boot-dot">{bootStep > i ? '✓' : '○'}</span>
              <span>{s}</span>
            </div>
          ))}
        </div>
    ...
    <main className="app-shell">
      ...
      <section className="hero-panel">
        <div>
          ...
          <p>{activeView === 'predict'
            ? 'Choose a disease, adjust patient values, and run the prediction to see risk status, confidence gauge, and model comparison.'
            : 'Review EDA, model evaluation, and PCA analysis generated from the original notebook.'}</p>
          <div className="hero-badges">
            <span>FastAPI backend</span><span>React dashboard</span><span>Saved ML models</span>
            <span>Patient history</span><span>PDF reports</span><span>Live risk preview</span>
          </div>
        </div>
          ...
          <section className="card input-card">
            ...
            <div className="fields">
              ...
              {filteredFields.map(field => {
                ...
                const isDanger = t && !field.type && inputs[field.key] >= t.danger
                const isWarn = t && !field.type && inputs[field.key] >= t.warn && inputs[field.key] < t.danger
                return (
                  <label
                    key={field.key}
