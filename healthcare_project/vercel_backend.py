...
Vercel (serverless) does not run uvicorn as a long-running server.
This module exposes a simple handler Vercel can call.

If your Vercel template expects a different export name, you may need
small adjustments in Vercel dashboard.
"""
...
app = FastAPI(
    title='Healthcare Risk API (Vercel)',
    description='Serverless backend service for Diabetes and Heart Disease prediction',
    version='1.0.0'
)

# Allow common local dev origins + Netlify previews by default.
# Update to your real frontend domain later if you want tighter security.
app.add_middleware(
    CORSMiddleware,
    allow_origins=[
        'http://localhost:5173',
...
def health_check():
    return {
        'status': 'ok',
        'service': 'Healthcare Risk API (Vercel)',
